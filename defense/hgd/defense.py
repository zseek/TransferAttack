...
def main():
    ...
    mean_torch = torch.from_numpy(np.array([0.485, 0.456, 0.406]).reshape([1,3,1,1]).astype('float32')).to(device)
    std_torch = torch.from_numpy(np.array([0.229, 0.224, 0.225]).reshape([1,3,1,1]).astype('float32')).to(device)
    mean_tf = torch.from_numpy(np.array([0.5, 0.5, 0.5]).reshape([1,3,1,1]).astype('float32')).to(device)
    std_tf = torch.from_numpy(np.array([0.5, 0.5, 0.5]).reshape([1,3,1,1]).astype('float32')).to(device)
    ...
    if not args.no_gpu:
        inresmodel = inresmodel.to(device)
        resmodel = resmodel.to(device)
        incepv3model = incepv3model.to(device)
        rexmodel = rexmodel.to(device)
    inresmodel.eval()
    resmodel.eval()
    incepv3model.eval()
    rexmodel.eval()
