...
def get_transform_list(checkpoint_path, images):
    ...
    encoder_model = encoder_model.eval().to(device)
    decoder_model = decoder_model.eval().to(device)
    predictor_model = predictor_model.eval().to(device)

    gamma = 15
    transform_index = list(random.randint(0, 19) for _ in range(4))
    one_hot_input = one_hot_emb(transform_index).view(-1, 80).to(device)
...
def get_sharpness_kernel():
    ...
    return torch.from_numpy(sharpness_kernel).to(device)
...
def input_shearX(input_tensor, delta=0.5):
    ...
    matrix = torch.tensor([1, factor, 0, 0, 1, 0]).view(2, 3).repeat(N, 1, 1).to(device)
...
def input_shearY(input_tensor, delta=0.5):
    factor = torch.FloatTensor(1).uniform_(-delta, delta)[0]
    N, C, W, H = input_tensor.size()
    matrix = torch.tensor([1, 0, 0, factor, 1, 0]).view(2, 3).repeat(N, 1, 1).to(device)
    size = torch.Size((N, C, W, H))
    grid = F.affine_grid(matrix, size)
    processed_image = F.grid_sample(input_tensor, grid)
    return processed_image
...
def input_translateX(input_tensor, delta=0.4):
    factor = torch.FloatTensor(1).uniform_(-delta, delta)[0]
    N, C, W, H = input_tensor.size()
    matrix = torch.tensor([1, 0, factor, 0, 1, 0]).view(2, 3).repeat(N, 1, 1).to(device)
    size = torch.Size((N, C, W, H))
    grid = F.affine_grid(matrix, size)
    processed_image = F.grid_sample(input_tensor, grid)
    return processed_image
...
def input_translateY(input_tensor, delta=0.4):
    factor = torch.FloatTensor(1).uniform_(-delta, delta)[0]
    N, C, W, H = input_tensor.size()
    matrix = torch.tensor([1, 0, 0, 0, 1, factor]).view(2, 3).repeat(N, 1, 1).to(device)
    size = torch.Size((N, C, W, H))
    grid = F.affine_grid(matrix, size)
    processed_image = F.grid_sample(input_tensor, grid)
    return processed_image
...
def input_reshape(input_tensor, delta=0.5):
    ...
    matrix = torch.tensor([scale_x, shear_x, translate_x, shear_y, scale_y, translate_y]).view(2, 3).repeat(N, 1, 1).to(device)
    size = torch.Size((N, C, W, H))
    grid = F.affine_grid(matrix, size)
    processed_image = F.grid_sample(input_tensor, grid)
    return processed_image
...
def input_gamma(input_tensor, delta=1.0):
    random_delta = torch.FloatTensor(input_tensor.size(0), 1, 1, 1).uniform_(1 - delta, 1 + delta).to(device)
...
def input_cutout(input_tensor):
    ...
    processed_image = transform(input_tensor).to(device)
    return processed_image

def input_invert(input_tensor): # 取反
    processed_image = -(input_tensor-0.5) + 0.5
    processed_image = -input_tensor.to(device)
    return processed_image
...
def input_hue(input_tensor, delta=0.2):
    ...
    processed_image = processed_image + mask.to(device)
    processed_image = torch.clamp(processed_image, 0.0, 1.0)
