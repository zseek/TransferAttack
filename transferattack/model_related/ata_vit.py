...
class ATA_ViT(Attack):
    ...
    def forward(self, data, label, **kwargs):
        ...
        for _ in range(self.epoch):
            ...
            loss =  (-torch.log(1-probs+1e-10) * torch.eye(data.shape[0]).to(device)).sum() / data.shape[0]
...
class ATTENTION_RIGION(torch.nn.Module):
    ...
    def generate_visualization(self, original_image, class_index=None):
        _, rollout = self.attribution_generator.generate_LRP(original_image.unsqueeze(0).to(device), method="transformer_attribution", index=class_index)
